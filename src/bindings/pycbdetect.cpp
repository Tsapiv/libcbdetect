...
PYBIND11_MODULE(pycbdetect, m) {
  ...
  m.def(
      "find_corners",
      [](const cv::Mat &image, Params params) {
        Corner corners;
        find_corners(image, corners, params);
        return corners;
      },
      "Detect corners in an image", "image"_a, "params"_a);

  m.def(
      "boards_from_corners",
      [](const cv::Mat &image, const Corner &corners, Params params) {
        std::vector<Board> boards;
        boards_from_corners(image, corners, boards, params);
        return boards;
      },
      "Detect checkerboard from corners", "image"_a, "corners"_a, "params"_a);
